...
const TechnicalBlogPost = ({ frontMatter, serializedMarkdown, readingTime }: BlogPostProps) => {
    if (!frontMatter) return <></>;

    return (
        <PublicPageLayout pageName="Technical Blog Posts">
...
export async function getStaticProps({ params }: Params) {
    const { frontMatter, markdownBody } = getPostBySlug("technical", params.slug);
...
export async function getStaticPaths() {
    const paths = getGenericStaticPaths("technical");
...
export default TechnicalBlogPost;
